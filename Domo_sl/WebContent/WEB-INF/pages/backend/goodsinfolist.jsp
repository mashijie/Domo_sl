...
<div>
	<ul class="breadcrumb">
		...
		<li><a
			href="${pageContext.request.contextPath }/GoodsInfo/GoodsInfoList">商品管理</a></li>
	</ul>
</div>
...
<div class="row-fluid sortable">
	<div class="box span12">
		...
		<div class="box-content">
			<form
				action="${pageContext.request.contextPath }/GoodsInfo/GoodsInfoList"
				method="post">
				<div class="searcharea">
					...
					状态： <select name="s_state" style="width: 100px;">
						...
						<c:choose>
							<c:when test="${s_state eq '' }">
								<option value="0">下架</option>
								<option value="1">上架</option>
							</c:when>
							<c:when test="${s_state eq 0}">
								<option value="0" selected="selected">下架</option>
								<option value="1">上架</option>
							</c:when>
							<c:when test="${s_state eq 1}">
						...
						<c:forEach items="${page.items}" var="goodsInfo">
							<tr>
								...
								<td class="center"><input type="checkbox"
									...
									state="${goodsInfo.state}" goodsinfoid="${goodsInfo.id}"
									<c:if test="${goodsInfo.state == 1}">checked="true"</c:if> /></td>
								...
								<td class="center"><a class="btn btn-success viewgoodsinfo"
									...
									id="${goodsInfo.id}" goodsName="${goodsInfo.goodsName}"> <i
				...
				<ul>
					<c:choose>
						<c:when test="${page.page == 1}">
							<li class="active"><a
								href="${pageContext.request.contextPath }/GoodsInfo/GoodsInfoList?p=1&s_goodsName=${s_goodsName}&s_state=${s_state}"
								title="首页">首页</a></li>
						</c:when>
						<c:otherwise>
							<li><a
					...
					<c:if test="${page.pageCount!=null}">
						<c:forEach begin='1' end='${page.pageCount}' var="num">
							<li><a
								href="${pageContext.request.contextPath }/GoodsInfo/GoodsInfoList?p=${num}&s_goodsName=${s_goodsName}&s_state=${s_state}"
					...
					<c:if test="${page.pageCount !=null}">
						<c:choose>
							...
							<c:otherwise>
								<li><a
									href="${pageContext.request.contextPath }/GoodsInfo/GoodsInfoList?p=${page.pageCount}&s_goodsName=${s_goodsName}&s_state=${s_state}"
									title="尾页">尾页</a></li>
							</c:otherwise>
						</c:choose>
					</c:if>
...
<div class="modal hide fade" id="addGoodsInfoDiv">
	<form
		action="${pageContext.request.contextPath }/GoodsInfo/addgoodsinfo"
		...
		<div class="modal-body">
			...
			<ul class="topul">
				...
				<li><label>状态：</label> <input type="radio" id="a_state"
					...
					id="a_state" name="state" value="2" />下架</li>
			</ul>
			<div class="clear"></div>
...
<div class="modal hide fade" id="modifyGoodsInfoDiv">
	<form
		action="${pageContext.request.contextPath }/GoodsInfo/modifygoodsinfo"
		enctype="multipart/form-data" method="post"
		...
		<div class="modal-body">
			...
			<ul class="topul">
				...
				<li><label>优惠价：</label> <input type="text" id="m_realPrice"
					...
					onkeyup="if(this.value==this.value2)return;if(this.value.search(/^\d*(?:\.\d{0,2})?$/)==-1)this.value=(this.value2)?this.value2:'';else this.value2=this.value;" />
					<span style="color: red; font-weight: bold;">*</span></li>
				<li><label>库存量：</label><input type="text" id="m_num" name="num"
					onkeyup="this.value=this.value.replace(/\D/g,'')"
					onafterpaste="this.value=this.value.replace(/\D/g,'')" /> <span
					style="color: red; font-weight: bold;">*</span></li>
	...
	<div class="modal-body">
		<ul class="topul">
			...
			<li><label>商品编号：</label><input type="text" id="v_goodsSN"
				name="goodsSN" readonly="readonly" /></li>
			...
			<li><label>单位：</label> <input type="text" id="v_unit"
				name="unit" readonly="readonly" /></li>
...
<script type="text/javascript"
	src="${pageContext.request.contextPath }/statics/localjs/goodsinfolist.js"></script>
