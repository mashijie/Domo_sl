	...
	$.ajax({
		...
		success: function(result){
			...
			if("failed" == m){
				...
				var state = m.state; 
				if(state == 1){
					$("#m_state").append("<span>状态：</span><input type=\"radio\" name=\"state\" checked=\"checked\" value=\"1\"/>上架<input type=\"radio\" name=\"state\" value=\"0\"/>下架");
//					$("#m_stateup").attr("checked","checked");
//					$("#m_statedown").removeAttr("checked");
					
				}else if(state == 0){
				    $("#m_state").append("<span>状态：</span><input type=\"radio\" name=\"state\" value=\"1\"/>上架<input type=\"radio\" name=\"state\" checked=\"checked\" value=\"0\"/>下架");
				}
...
$('.viewgoodsinfo').click(function(e){
	...
	$.ajax({
		...
		success: function(result){
			m = eval('(' + result + ')'); //result==""xxx""   m="xxx"
			if("failed" == m){
				alert("操作超时！");
			}else if("nodata" == m){
				alert("没有数据！");
...
$('.delgoodsinfo').click(function(e){
	...
	if(confirm("您确定要删除【"+d_goodsName+"】这个商品吗？")){
		//delete
		$.post("delgoodsinfo",{'delId':d_id},function(result){
			if("success" == result){
				alert("删除成功！");
				window.location.href="GoodsInfoList";
...
$("#a_goodsSN").blur(function(){
	...
	if(agsn != ""){
		$.post("goodsSNisexit",{'goodsSN':agsn,'id':'-1'},function(result){
			if(result == "repeat"){
...
$("#m_goodsSN").blur(function(){
	...
	if(agsn != ""){
		$.post("goodsSNisexit",{'goodsSN':agsn,'id':$("#m_id").val()},function(result){
			if(result == "repeat"){
