...
$("#modifytypeCode").blur(function(){
	...
	if(mlc != ""){
		$.post("typecodeisexit",{'typeCode':mlc,'id':$("#modifydicid").val()},function(result){
			if(result == "repeat"){
		...
		success: function(result){
			if("failed" == result){
				...
				for(var i=0; i<result.length;i++){
					...
					str += "<img class=\"modifyDicValue\" modifytypecode="+result[i].typeCode+" modifytypename="+result[i].typeName+" id=\""+result[i].id+"\" valueid=\""+result[i].valueId+"\" valuename=\""+result[i].valueName+"\" src=\"../statics/img/ico10.png\"> <img class=\"delDicValue\" deltypename="+result[i].typeName+" id=\""+result[i].id+"\" valueid=\""+result[i].valueId+"\" valuename=\""+result[i].valueName+"\"  src=\"../statics/img/linkdel.png\">";
					str += "</div>";
...
$("#modifyDicExeBtn").click(function(){
	...
	if(dic.typeCode == ""){
		...
		$.ajax({
			...
			success: function(result){
				if(result != "" && "success" == result){
					window.location.href = "dicmanage.html";
				}else if("failed" == result){
					tip.css("color","red");
					tip.html("数据字典修改失败！请重试。");
				}else if("nodata" == result){
...
$(".delMainDic").click(function(){
	...
	if(confirm("删除操作会删除\""+dictypename+"\"下所有的字典值，您确定要删除\""+dictypename+"\"吗？")){
		...
		$.ajax({
			...
			success: function(result){
				if(result != "" && "success" == result){
					window.location.href = "/Domo_sl/Dictionary/dicmanage";
				}else if("failed" == result){
					alert("数据字典删除失败！请重试。");
				}else if("nodata" == result){
