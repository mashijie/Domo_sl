		...
		success: function(result){
			...
			if("failed" == m){
				...
				if(m_idcardpicpath == null || m_idcardpicpath == "" ){
					...
					$("#m_idPic").append("<p><span onclick=\"delpic('"+m.id+"','m_idPic','m_uploadbtnID',this,'"+m_idcardpicpath.replace(/\"/g,"")+"','m_fileInputIDPath','m_fileInputID');\">x</span><img src=\""+m_idcardpicpath.replace(/\"/g,"")+"?m="+Math.random()+"\" /></p>");
					$("#m_uploadbtnID").hide();
				}
				...
				if(m_bankpicpath == null || m_bankpicpath == "" ){
					...
					$("#m_bankPic").append("<p><span onclick=\"delpic('"+m.id+"','m_bankPic','m_uploadbtnBank',this,'"+m_bankpicpath.replace(/\"/g,"")+"','m_fileInputBankPath','m_fileInputBank');\">x</span><img src=\""+m_bankpicpath.replace(/\"/g,"")+"?m="+Math.random()+"\" /></p>");
	...
	$.ajax({
		...
		success: function(result){
			...
			if("failed" == m){
				...
				var isstart = m.isStart;
				if(isstart == '1'){
					$("#v_isstart").html("<option value=\"1\" selected=\"selected\">启用</option><option value=\"2\">不启用</option>");
				}else{
					$("#v_isstart").html("<option value=\"1\">启用</option><option value=\"2\" selected=\"selected\">不启用</option>");
				}
...
$('.delmember').click(function(e){
	...
	if(confirm("您确定要删除【"+d_logincode+"】这个用户吗？")){
		//delete										  
		$.post("deluser",{'delId':d_id,'delIdCardPicPath':d_idcardpicpath,'delBankPicPath':d_bankpicpath,'delUserType':'1'},function(result){
			if("success" == result){
				alert("删除成功！");
				window.location.href="/Domo_sl/Member/memberlist";
			}else{
				alert("删除失败！");
			}
		},'text');
	    ...
	    ({ 
	           ...
	           success: function(data) { 
	        	   ...
	        	   if(data == "1"){
	        		   ...
	        		   $("#"+t3+"").append("<p><span onclick=\"delpic('"+flag+"','"+t3+"','"+t2+"',this,'"+data.replace(/\"/g,"")+"','"+t4+"','"+t1+"');\">x</span><img src=\""+data.replace(/\"/g,"")+"?m="+Math.random()+"\" /></p>");
	        		   $("#"+t2+"").hide();
	        		   $("#"+t4+"").val(data);
	        		   $("input[name='"+t1+"']").change(function(){
	        			   var fn = $("input[name='"+t1+"']").val(); 
...
function delpic(id,closeSpan,uploadBtn,obj,picpath,picText,fileinputid){
	//delete
	$.post("delpic",{'id':id,'picpath':picpath},function(result){
		if("success" == result){
			alert("删除成功！");
			$('#'+picText).val('');
