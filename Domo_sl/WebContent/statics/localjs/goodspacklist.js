$('.addGoodsPack').click(function(e){
	window.location.href="/Domo_sl/GoodSpack/addgoodspack";
});

$('.modifygoodspack').click(function(e){
	var id = $(this).attr('id');
	window.location.href="/Domo_sl/GoodSpack/modifygoodspack?id="+id;
});

$('.viewgoodspack').click(function(e){
	var id = $(this).attr('id');
	window.location.href="/Domo_sl/GoodSpack/viewgoodspack?id="+id;
});
...
$('.delgoodspack').click(function(e){
	...
	if(confirm("您确定要删除【"+goodsPackName+"】这个商品套餐吗？")){
		//delete
		$.post("delgoodspack",{'delId':d_id},function(result){
			if("success" == result){
				alert("删除成功！");
